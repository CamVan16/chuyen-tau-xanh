    ...
    <form>
        <div class="row mb-3">
            ...
            <div class="col-md-3">
                ...
                <input type="date" id="ngayDi" class="form-control" disabled>
            </div>
            <div class="col-md-3">
                <label for="ngayVe">Ngày về</label>
                <input type="date" id="ngayVe" class="form-control" disabled>
            </div>
        </div>
        <div class="form-group">
            <button type="submit" class="btn btn-primary" id="searchBtn" disabled>Tìm kiếm</button>
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.suggestion-item, .station-btn', function() {
            ...
            if (type === 'gaDi') {
                ...
                $('#gaDen').val(selectedGaDen);
                $('#gaDenSuggestions').empty().hide();
                $('#ngayDi').prop('disabled', false);
            }
        });

        // Kích hoạt ngày về sau khi chọn ngày đi
        $('#ngayDi').on('change', function() {
            const selectedNgayDi = $(this).val();
            if (selectedNgayDi) {
                $('#ngayVe').prop('disabled', false);
                $('#ngayVe').attr('min', selectedNgayDi); // Đảm bảo ngày về >= ngày đi
            }
        });

        // Kích hoạt nút Tìm kiếm sau khi chọn ngày về
        $('#ngayVe').on('change', function() {
            const selectedNgayVe = $(this).val();
            if (selectedNgayVe) {
                $('#searchBtn').prop('disabled', false);
            }
        });
        ...
        $('form').on('submit', function(event) {
            ...
            const ngayVe = $('#ngayVe').val();
            alert(`Tìm kiếm: Ga đi - ${gaDi}, Ga đến - ${gaDen}, Ngày đi - ${ngayDi}, Ngày về - ${ngayVe}`);
        });
    });
</script>
@endsection --}}
