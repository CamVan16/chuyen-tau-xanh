...
class VNPayController extends Controller
{
    public function processPaymentExchange(Request $request)
    {
        ...
        session()->flash('exchange', $exchange);
        ...
        $vnp_Returnurl = "http://localhost:8000/payment/vnpay/callback/exchange";
    ...
    public function handleVNPayResponseExchange(Request $request)
    {
        ...
        if ($request->get('vnp_ResponseCode') === "00") {
            return redirect()->route('transaction.showInfoExchange', [
                ...
                'exchange' => base64_encode(json_encode($exchange)),
            ]);
        } else {
            return redirect()->route('transaction.showInfoExchange', [
                'status' => 'error',
